...
window.onload = function () {
    ...
    if ((theUA.match(/msie\s\d+/) && theUA.match(/msie\s\d+/)[0]) || (theUA.match(/trident\s?\d+/) && theUA.match(/trident\s?\d+/)[0])) {
        ...
        let browerNav = document.createElement('div');
    ...
    let includesConstructor = Vue.extend({
        template :
                ...
                mainHandle.constDom.functionGroup.http.post(
                    ...
                    function (data) {
                        if(data['data']['res'] === static_data.response.requestSuccess)
                        {
                            ...
                            for(let i in includes.postDepartment)
                            {
...
window.onscroll = function () {
    function myTry() {
        let documentTop = mainHandle.constDom.functionGroup.getDocumentTop();
        let scrollHeight = mainHandle.constDom.functionGroup.getScrollHeight();
        let windowHeight = mainHandle.constDom.functionGroup.getWindowHeight();
